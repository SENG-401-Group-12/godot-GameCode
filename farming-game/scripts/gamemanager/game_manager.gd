...
func _ready() -> void:
	...
	call_deferred("_start_next_wave") # wait until all _ready() calls are finished before starting
...
func _start_next_wave() -> void:
	...
	game_ui._show_message("Wave %d started. %d hungry customers are waiting." % [current_wave, wave_customer_count])

func _check_wave_complete():
	if allowed_misses > 0 and wave_missed_count >= allowed_misses:
		game_finished = true
		game_ui._show_message("Wave Failed. Too many customers left hungry.")
		
	if customer_spawner.get_remaining_count() == 0 and !game_finished:
		waiting_for_next_wave = true
		game_ui._show_message("Wave Complete! 5s until next wave.")
		
		await get_tree().create_timer(5.0).timeout # delay 5s before staring next wave
		_start_next_wave()
